...
function debug_step_up() {
    if (debug_index < debug_pitches.length - 1) {
        debug_index += 1;
    }
    debug_update_pitch();
}

function debug_update_pitch() {
    var pitch = debug_pitches[debug_index];
    var freq = frequency_for_pitch(pitch);
    var message = "pitch " + debug_index + "/" + debug_pitches.length + " is " + pitch + " @ " + freq;
    if (!debug_playing) {
        message += " (paused)";
    }
    document.getElementById('debug-current').innerHTML = message;
    audio_change(freq);
}
